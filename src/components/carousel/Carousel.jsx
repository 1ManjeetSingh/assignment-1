...
const Carousel = () => {
  ...
  return (
    ...
    >
      <div className="flex justify-center items-center relative overflow-hidden" style={{ height: '350px', perspective: '1500px' }}>
        <div
          className="flex absolute transition-transform duration-500 transform"
          style={{
            transform: `rotateY(${(currentSlide / slidesData.length) * 360}deg)`,
            transformStyle: 'preserve-3d',
            width: '700px',
          }}
        >
          {slidesData.map((slide, index) => {
            const angle = (360 / slidesData.length) * index;
            ...
            return (
              <div
                key={index}
                className={`absolute transition-all duration-500 transform ${isActive ? 'scale-110 z-20 opacity-100' : 'scale-100 z-10 opacity-70'}`}
                style={{
              ...
              >
                <div className="p-6 bg-gradient-to-r border-purple-600 border-2 from-blue-950 to-cyan-950 shadow-xl text-white rounded-lg flex justify-center items-center">
                  ...
                  <div className="flex items-center justify-center mb-4 w-48 ">{slide.icon}</div>
                </div>
              </div>
            );
          })}
